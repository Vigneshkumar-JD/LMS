...
public class DeleteCourseServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            PreparedStatement stmt = conn.prepareStatement("DELETE FROM newlyaddedcourses WHERE course_id = ?");
            stmt.setInt(1, id);
            ...
            out.print("{\"status\":\"success\",\"message\":\"Course deleted successfully\"}");
        } catch (Exception ex) {
            // send an error response back to the client
            response.setContentType("text/html");
            PrintWriter out = response.getWriter();
            out.print("{\"status\":\"error\",\"message\":\"Course could not be deleted\"}");
        }
    }
}